...
| T-40 | Session key with empty scopes silently accepted | Elevation of Privilege | Low | Medium | **Fail-closed scope enforcement (new in 0.2.0):** `execute_with_session` rejects any session key whose `scopes` list is empty (`Unauthorized`) instead of returning `Ok` unconditionally. A key granted zero capabilities must not be able to execute anything. Covered by unit test `test_execute_with_session_rejects_empty_scopes`. |
...
- **Session-key payload dispatch** — `execute_with_session` validates and fail-closed-gates the session key (T-40) but does not decode or dispatch `payload`; per-method scope checking against a decoded payload remains future work (see [aa_sequence_diagram.md](aa_sequence_diagram.md)). A session key with a **non-empty** scope list is currently accepted without verifying the payload targets a permitted method.
